...
const AIOApp = (() => {
  ...
  async function renderDashboard(){
    ...
    // CONDIÕES ATUAIS — tempo real (Caraúbas-PB)
    if(c){
      ...
      setVal('dashTempNote', `Sensação térmica: ${c.apparent_temperature.toFixed(1)}°C`);
      setVal('dashSunrise', new Date(daily.sunrise[0]).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}));
      setVal('dashSunset', new Date(daily.sunset[0]).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}));
      setVal('dashCloudCover', c.cloud_cover + '%');
    }
    ...
    if(evo){
      evo.setOption({
        ...
        series:[
          ...
          {name:'Umidade (Moisture)', type:'line', smooth:true, data:AIO.indices.moisture, itemStyle:{color:'#8a7dff'}},
          {name'Solo Exposto (Barren)', type:'line', smooth:true, data:AIO.indices.barren, itemStyle:{color:'#ffb545'}},
        ]
      });
    }
    ...
    // Radar de sustentabilidade - todos os 5 KPIs
    const sus = ec('chartSustain');
    if(sus){
      const kpis = [
        {name:'Cobertura Vegetal', value:AIO.kpi.cobertura_vegetal_pct},
        {name:'Retenção Hídrica', value: AIO.kpi.taxa_mortalidade_pct},
        {name:'Fitossanidade', value: AIO.kpi.area_recuperada_pct},
        {name:'Recuperação Solo', value: 71}, // valor padrão
        {name:'Biodiversidade', value: 49}   // valor padrão
      ];
      sus.setOption({
        radar:{ indicator:kpis.map(k=>({name:k.name, max:100})), axisName:{color:'#5f7c90', fontSize:10}, splitLine:{lineStyle:{color:'rgba(120,200,255,.14)'}}, splitArea:{show:false}, axisLine:{lineStyle:{color:'rgba(120,200,255,.14)'}} },
        series:[{type:'radar', data:[{value:kpis.map(k=>k.value), itemStyle:{color:'#3fe0ff'}, lineStyle:{color:'#3fe0ff'}}]}]
      });
    }
  }
  ...
  // ---------- MAPA ----------
  let mapInst=null, layers={}, polyLayer=null, uploadedLayer=null, fireLayer=null, riverLayer=null, meteoLayer=null;
  function renderMapa(){
    ...
    // River course layer (Q = A × V hydrology model)
    const riverCoords = [
      [LAT+0.01,LON-0.01], [LAT+0.005,LON-0.005], [LAT-0.005,LON], [LAT-0.01,LON+0.01],
      [LAT-0.005,LON+0.015], [LAT+0.005,LON+0.02], [LAT+0.01,LON+0.015], [LAT+0.015,LON+0.01]
    ];
    const riverStyle = {
      color: '#12e0b0',
      weight: 3,
    ...
    let riverWidth = 3;
    function updateRiverFlow(Q, V) {
      riverWidth = Math.max(1, Math.min(8, Math.abs(Q) / 5 + 2));
      riverStyle.weight = riverWidth;
      riverStyle.color = Q > 0 ? '#12e0b0' : '#ff4d5e';
      if(riverLayer){ riverLayer.setStyle(riverStyle); }
    }
    riverLayer = L.polyline(riverCoords, riverStyle).bindPopup(`<b>Rio Paraíba — trecho Congo→Caraúbas</b><br>${AIO.hydro.trecho_km} km · Área: ${AIO.hydro.geom.area_m2} m² · Q: -- m³/s`).addTo(mapInst);

    // Meteorologic station marker layer
    meteoLayer = L.marker([LAT, LON], {icon: L.icon({iconUrl:'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/home.png', iconSize:[32,32], iconAnchor:[16,32]})}).bindPopup(`
      <b>Estação Meteorológica Open-Meteo</b><br>
      Conectada via API em tempo real<br>
    ...
    document.querySelectorAll('.chip-toggle').forEach(chip=>{
      chip.addEventListener('click', ()=>{
        ...
        if(key==='hydro'){
          if(mapInst.hasLayer(riverLayer)){ mapInst.removeLayer(riverLayer); chip.classList.remove('on'); }
          else { riverLayer.addTo(mapInst); chip.classList.add('on'); }
          return;
        }
        if(key==='meteo'){
          if(meteoLayer){ mapInst.removeLayer(meteoLayer); meteoLayer=null; chip.classList.remove('on'); }
          else { meteoLayer.addTo(mapInst); chip.classList.add('on'); }
          return;
        }
  ...
  // ---------- MÉTÉOROLOGIC MARKERS ----------
  function addMeteoMarkers(){
    // Clear existing markers
    if(meteoLayer){
      mapInst.removeLayer(meteoLayer);
      meteoLayer = null;
    }
    ...
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${LAT}&longitude=${LON}&current_weather=true`)
      ...
      .then(data=>{
        ...
        meteoLayer = L.marker([LAT, LON], {icon: L.icon({iconUrl:'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/home.png', iconSize:[32,32], iconAnchor:[16,32]})}).bindPopup(`
          ...
          Temp: ${c.temperature}°C · Vento: ${c.wind_speed} km/h · Direção: ${c.wind_direction}° · Pressão: ${c.pressure} hPa<br>
          Atualizado: ${new Date().toLocaleTimeString('pt-BR')}
        `).addTo(mapInst);
        // Add compass direction marker
        const dirs = ['N','NE','E','SE','S','SO','O','NO'];
        const dirIndex = Math.round(c.wind_direction/45)%8;
        const dirCenter = dirs[dirIndex];
        const dirCoords = [
          [LAT+0.002,LON], [LAT+0.0015,LON+0.001], [LAT+0.001,LON+0.002], [LAT+0.0015,LON+0.003],
          [LAT,LON+0.003], [LAT-0.0015,LON+0.003], [LAT-0.002,LON], [LAT-0.0015,LON-0.001]
        ];
        const dirPoly = L.polygon(dirCoords, {color:'#ffb545', fillColor:'#ffb545', fillOpacity:0.1, weight:2}).addTo(mapInst);
        dirPoly.bindPopup(`Direção do vento: ${dirCenter}`);
      })
      .catch(()=>{});
  }
